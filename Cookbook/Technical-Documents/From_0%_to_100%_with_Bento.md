...
```plain
 12 Aug 2019 - Renderer has to be a struct as there is a problem 
 with classes & memory management as classes are not 
 deallocated. It may be fixed someday but it's still the case 
...
```
Very often there is only a single Section on the screen. 
In that case, we name this Section's id as "first".
```
...
```
In our codebase, there is an additional scheme & target called "GalleryApp".
You can find there all Design Library components which have been written so far.
```
