...
Certificates and provisioning profiles for enterprise builds (Hockey App) or release builds (Testflight and the App Store) can also be downloaded. Normally, this is not needed – as those tasks are usually executed on Circle CI and not locally – but might be necessary if you want to run UI tests locally or work on updating our fastlane build scripts.
...
```shell
 $ fastlane match release --git_branch "appstore team id" --readonly
```
...
Recommended practice is to have on branch per team. Default values for the git URL, branch, app identifier and build type are read from the `Matchfile`.
...
There were some problems (March 2019) with running `bundle exec fastlane spaceship`. It might be necessary to manually install some gems in the `~/.fastlane` directory.
